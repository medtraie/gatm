...
import { useStickyState } from '@/contexts/AppContext';

const OilBarrelsWidget: React.FC = () => {
  const navigate = useNavigate();
  
  const [barrelCount, setBarrelCount] = useStickyState<number>(5, 'oilManagement.barrelCount');
  const [purchasesLS] = useStickyState<Array<{ date: string; quantityLiters: number }>>([], 'oilManagement.purchases');
  const [consumptionsLS] = useStickyState<Array<{ date: string; quantityLiters: number }>>([], 'oilManagement.consumptions');
  const [baseStock] = useStickyState<number>(0, 'oilManagement.baseStockLiters');
  const oilLevel = useMemo(() => {
    const totalPurchased = baseStock + purchasesLS.reduce((sum, p) => sum + (p.quantityLiters || 0), 0);
    const totalConsumed = consumptionsLS.reduce((sum, c) => sum + (c.quantityLiters || 0), 0);
    const currentStock = Math.max(0, totalPurchased - totalConsumed);
    const capacity = 1000;
    return Math.min((currentStock / capacity) * 100, 100);
  }, [baseStock, purchasesLS, consumptionsLS]);
