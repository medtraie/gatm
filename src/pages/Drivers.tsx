...
const Drivers = () => {
  const { drivers, bottleTypes, transactions } = useApp();
                    ...
                    <TableRow key={driver.id} className="hover:bg-slate-50/80 transition-colors border-b border-slate-50 last:border-0">
                      ...
                      <TableCell className="py-4">
                        <span className="text-emerald-600 font-bold">
                          {driver.advances.toLocaleString()} <span className="text-[10px] opacity-70">DH</span>
                        </span>
                      </TableCell>
                      <TableCell className="py-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card className="border-none bg-slate-50 p-6 shadow-inner">
                      ...
                      <div className="text-3xl font-bold text-emerald-600">
                        {selectedDriver.advances.toLocaleString()} <span className="text-sm">DH</span>
                      </div>
                    </Card>
                    ...
                    <Card className="p-6 border-slate-100 shadow-sm">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-100">
                          {selectedDriver.advances > 0 ? Math.min((selectedDriver.advances / Math.abs(selectedDriver.debt)) * 100, 100).toFixed(1) : 0}% Complété
                        </Badge>
                      </div>
                            ...
                            driverTransactions.map((tx, index) => (
                              <TableRow key={`${selectedDriver.id}-${tx.id ?? index}`} className="hover:bg-slate-50/50 border-b border-slate-50 last:border-0">
                                ...
                                <TableCell className="py-4 font-bold">
                                  {tx.amount.toLocaleString()} <span className="text-[10px] opacity-70">DH</span>
                                </TableCell>
                                <TableCell className="py-4 text-slate-600 italic text-sm">{tx.description}</TableCell>
                              </TableRow>
                            ))
                          ) : (
                            <TableRow>
