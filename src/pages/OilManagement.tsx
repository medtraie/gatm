...
import { useApp, useStickyState } from "@/contexts/AppContext";
...
function OilManagement() {
  ...
  const [barrelCount, setBarrelCount] = useStickyState<number>(2, "oilManagement.barrelCount");
  ...
  const [barrelsConfig, setBarrelsConfig] = useStickyState<Array<{ name: string; capacityLiters: number }>>(
    Array.from({ length: barrelCount }, (_, i) => ({ name: `Baril ${i + 1}`, capacityLiters: 220 })),
    "oilManagement.barrelsConfig"
  );
  useEffect(() => {
  ...
  const [purchasesLS, setPurchasesLS] = useStickyState<Array<{ date: string; quantityLiters: number }>>([], "oilManagement.purchases");
  const [consumptionsLS, setConsumptionsLS] = useStickyState<Array<{ date: string; quantityLiters: number }>>([], "oilManagement.consumptions");
  const [baseStockLiters, setBaseStockLiters] = useStickyState<number>(0, "oilManagement.baseStockLiters");
  ...
  const [purchaseCapacities, setPurchaseCapacities] = useStickyState<Record<string | number, number>>({}, "oilManagement.purchaseCapacities");
  const [consumptionCapacities, setConsumptionCapacities] = useStickyState<Record<string | number, number>>({}, "oilManagement.consumptionCapacities");
  ...
  useEffect(() => {
    const lsPurchases = (oilPurchases || []).map((p) => ({ date: p.date, quantityLiters: (p.quantity || 0) * (purchaseCapacities[p.id] || 220) }));
    setPurchasesLS(lsPurchases);
  }, [oilPurchases, purchaseCapacities]);
  useEffect(() => {
    const lsConsumptions = (oilConsumptions || []).map((c) => ({ date: c.date, quantityLiters: (c.quantity || 0) * (consumptionCapacities[c.id] || 220) }));
    setConsumptionsLS(lsConsumptions);
  }, [oilConsumptions, consumptionCapacities]);
  ...
  const addPurchase = () => {
    ...
    setPurchaseCapacities(prev => ({ ...prev, [newPurchase.id]: purchaseCapacityLiters }));

    // Mirror purchase in liters for dashboard widget
    setPurchasesLS(prev => ([...prev, {
  ...
  const addConsumption = () => {
    ...
    // Persist capacity for this row
    setConsumptionCapacities(prev => ({ ...prev, [newConsumption.id]: consumptionCapacityLiters }));
