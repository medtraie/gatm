...
const Factory = () => {
  ...
  const [factoryOperations, setFactoryOperations] = useStickyState<FactoryOperation[]>([], 'factoryOperations');
  const [debtSettlements, setDebtSettlements] = useStickyState<DebtSettlement[]>([], 'debtSettlements');
  const [invoices, setInvoices] = useStickyState<Invoice[]>([], 'factoryInvoices');
  ...
  const handleReturnFromFactory = () => {
    ...
    setFactoryOperations(prev => prev.map(op => 
      op.id === operationId 
        ? { ...op, receivedBottles, debtChange, receivedDate: returnForm.date ? returnForm.date.toISOString() : new Date().toISOString() }
        : op
    ));
